...
class RegionCurveData(BaseModel):
    ...
    available_percentiles: List[int]
...
class NormativeResponse(BaseModel):
    ...
    metadata: Dict
    percentile_curves: Optional[Dict[str, RegionCurveData]] = None
...
def run_normative_modeling(volumes: Dict, metadata: Dict) -> Dict:
    ...
    try:
        ...
        outlier_regions = []
        region_analyses = {}
        percentile_curves_data = {}  # New: Store curve data for frontend
            ...
            try:
                ...
                if percentiles_folder.exists():
                    ...
                    if percentile_file.exists():
                        ...
                        available_percentiles = [c for c in df_percentiles.columns if isinstance(c, int)]
                        selected_pcts = [p for p in [1, 5, 10, 25, 50, 75, 90, 95, 99] if p in available_percentiles]
                        
                        if selected_pcts:
                            # Extract age values and percentile curves
                            ages = df_percentiles["Age"].astype(float).tolist()
                            percentile_curves = {}
                            
                            for pct in sorted(selected_pcts):
                                values = df_percentiles[pct].astype(float).values
                                percentile_curves[str(pct)] = values.tolist()
                            
                            # Store curve data for frontend
                            percentile_curves_data[region] = {
                                "region_name": region,
                                "ages": ages,
        ...
        return {
            ...
            "percentile_curves": percentile_curves_data  # Add curve data for frontend
        }
...
async def normative_modeling_inference(
    ...
    try:
        ...
        response = NormativeResponse(
            ...
            metadata=metadata,
            percentile_curves=normative_result.get("percentile_curves", {})
        )
