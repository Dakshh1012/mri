...
async def brain_age_prediction_route(
    ...
    if "application/json" in content_type:
        try:
            ...
            print(f"[Brain-Age] Received JSON payload: {payload}")
            url_in = payload.get('nifti_url')
            # Override form parameters with JSON values
            a = payload.get('age')
            g = payload.get('gender')
            print(f"[Brain-Age] Extracted: url={url_in}, age={a}, gender={g}")
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Invalid JSON body: {str(e)}")
    
    # Case A: URL-driven flow (JSON body with nifti_url)
    if url_in:
        # Extract age/gender from metadata_json if still needed
        ...
        if a is None:
            raise HTTPException(status_code=400, detail="Age is required (provide in JSON body or metadata_json)")
        if not g:
            raise HTTPException(status_code=400, detail="Gender is required (provide in JSON body or metadata_json)")
        
        # Normalize gender
        gnorm = _normalize_gender(g)
        if gnorm not in {"M", "F"}:
        ...
        try:
            ...
            print(f"Downloading NIfTI from: {url_in}")
            ...
            # Run pipeline
            print(f"Running pipeline for {participant_id}, age={a}, gender={gnorm}")
            ...
            response = BrainAgeResponse(
                job_id=job_id,
                participant_id=participant_id,
                status=brainage_result.get("status", "success"),
                chronological_age=brainage_result.get("chronological_age", float(a)),
                predicted_brain_age=brainage_result.get("predicted_brain_age", float(a)),
                brain_age_gap=brainage_result.get("brain_age_gap", 0.0),
            ...
            result_dict = response.model_dump()
            save_results_locally(result_dict, "brain_age", participant_id, job_id, RESULTS_DIR)
            return response
            
        except HTTPException:
            raise
        except Exception as e:
            print(f"Error in brain age prediction: {e}")
            raise HTTPException(status_code=500, detail=f"Processing failed: {str(e)}")
        finally:
            if job_dir.exists():
    ...
    if metadata_json:
        try:
            ...
            a = float(metadata.get('age')) if a is None else a
            g = str(metadata.get('gender')) if not g else g
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Invalid JSON metadata: {str(e)}")
    ...
    g = _normalize_gender(g)
    if a is None or not g:
        raise HTTPException(status_code=400, detail="Age and gender are required (provide as form fields or in metadata_json)")
    if g not in ['M', 'F']:
        raise HTTPException(status_code=400, detail="Gender must be 'M' or 'F'")

    return await brain_age_prediction_inference(
        nifti_file, float(a), g,
